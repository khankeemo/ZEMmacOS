...
class buildMacOSInstallApp:
    def __init__(self):
        ...
        # Check if we're on macOS - show warning but don't exit (for VM users)
        if not sys.platform == "darwin":
            self.u.head("WARNING")
            print("")
            print("This script requires macOS to run.")
            print("You can run it in a macOS virtual machine (VMware/VirtualBox).")
            print("The script will now show instructions for manual conversion.")
            print("")
            self.u.grab("Press [enter] to continue with instructions...")
            self.show_instructions()
            return

    def show_instructions(self):
        """Show instructions for manual macOS app creation on Windows"""
        self.u.head("INSTRUCTIONS")
        print("")
        print("To create a macOS installer app on Windows:")
        print("")
        print("1. Download the macOS files using ZEMmacOS Library")
        print("2. Copy the downloaded folder to a macOS machine or VM")
        print("3. On macOS, run: python BuildmacOSInstallApp.py")
        print("4. Drag and drop the downloaded folder when prompted")
        print("5. The script will create Install macOS.app")
        print("")
        print("Alternative: Use a real Mac or macOS VM")
        print("")
    ...
    def main(self):
        while True:
            ...
            try:
                ...
                print("")
                print("To create a bootable USB, use:")
                print("  sudo {}".format(os.path.join(install_app,"Contents","Resources","createinstallmedia")))
                print("  --volume /Volumes/MyUSB")
                print("")
                self.u.grab("Press [enter] to return...")
